...
#
# Syntaxe : deb_build.sh [nom_paquet] [version_plume_pg] [revision_paquet]
#-------------------------------------------------------------------------
...
# Usage
usage() {
  echo "Usage : $0 [nom_paquet] [version_plume_pg] [revision_paquet]"
  exit 1
}
[ $# -lt 3 ] && usage
...
mkdir -p $DEB_LIB
cp -f $PROJECT_DIR/postgresql/archives/plume_pg--*--*.sql $PROJECT_DIR/postgresql/plume_pg--*.sql $PROJECT_DIR/postgresql/plume_pg.control $DEB_LIB
